...
var main = module.exports = function() {
        ...
        Object.keys(struct).sort().forEach(function(routePart) {
            ...
            if (block.url && block.params) {
                ...
                testSections[section].push(TestHandlerTpl
                    ...
                    .replace("<%method%>", block.method.toLowerCase())
                    .replace("<%name2%>", "'" + block.url + "'")
                    .replace("<%params%>", "{ ".concat(getParams(section, block.params, "").replace(/[\{\}]/g, '').trim(), " }"))
                    .replace("<%funcName%>", section + "." + funcName)
                    .replace("<%params2%>", getParams(section, block.params, "            "))
                );
            }
            else {
    ...
    function getParams(section, paramsStruct, indent) {
        ...
        for (var i = 0, l = params.length; i < l; ++i) {
            ...
            // only use required params
            if (!def.required) {
                continue;
            }

            // use param default or test value
            var testValue = def.testValue;
            if (typeof(testValue) === "undefined") {
                testValue = def.default;
            }

            // some exceptions
            if (typeof(testValue) === "undefined") {
                if (section === "gists" && paramName === "id") {
                    testValue = "00ccfb395ec8410daec2";
                }
            }

            values.push(indent + "    " + paramName + ": \"" + testValue + "\"");
        }
        return "{\n" + values.join(",\n") + "\n" + indent + "}";
    }
